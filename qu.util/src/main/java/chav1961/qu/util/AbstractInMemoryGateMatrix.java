...
abstract class AbstractInMemoryGateMatrix extends AbstractGateMatrix {
	AbstractInMemoryGateMatrix(final GateMatrixType type, final long width, final long height, final boolean parallelModeOn) {
	...
	protected static void calcAndAdd(final float leftReal, final float leftImage, final ComplexOp op, final float rightReal, final float rightImage, final float[] result, final int resultIndex) {
		switch (op) {
			case ADD		:
